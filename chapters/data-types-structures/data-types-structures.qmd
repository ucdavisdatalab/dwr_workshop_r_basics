...
This chapter focuses on the foundational knowledge and skills you'll need in
order to use R effectively in the long term. Specifically, it's a deep dive
into R's various data types and data structures.


## Types & Classes {#sec-data-types-classes}
...
We can categorize data into **types** based on sets of shared characteristics.
This makes it easier to reason about how to transform, analyze, and present the
data. Some widely-understood data types are:

* **Categorical**
    + **Nominal** -- data separated into specific categories, with no order.
      For example, hair color (red, brown, blonde, ...) is categorical.
    + **Ordinal** -- data separated into specific categories, with an order.
      For example, school level (elementary, middle, high, college) is ordinal.
* **Numerical**
...
1. The **class** of an R object describes what the object does, or the role
   ...
   have more than one class. The `class` function, which debuted in
   @sec-ca-least-terns, returns the classes of its argument.
...
Of the two, classes are more important than types. If you aren't sure what an
object is, checking its classes should be the first thing you do.
...
| matrix     | 2-dim ordered collection of homogeneous elements (@sec-matrices-arrays)
| array      | $p$-dim ordered collection of homogeneous elements (@sec-matrices-arrays)
| factor     | Vector of categorical values (@sec-factors)
...
## Matrices & Arrays {#sec-matrices-arrays}
...
When it reads a dataset, R usually can't tell which features are categorical.
...
Let's think about which features are categorical in least terns dataset. To
refresh your memory of what's in the dataset, take a look at the structural
summary:
...
dataset. It's implied that the entries are missing due to how the data was
